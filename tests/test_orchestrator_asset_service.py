...
def test_inline_cover_asset_has_nosniff_and_is_forced_to_attachment() -> None:
    ...
    store = Mock()
    store.head_object.return_value = {
        "ContentLength": 24,
...
def _unsafe_cover_asset_stream_dependencies() -> tuple[Mock, Mock, uuid.UUID, uuid.UUID]:
    ...
    return db, store, task_id, asset_id
...
def test_actual_asset_responses_always_harden_unsafe_cover_headers(
    ...
    db, store, task_id, asset_id = _unsafe_cover_asset_stream_dependencies()
...
def test_delete_final_asset_defers_object_delete_until_a_later_retry() -> None:
    ...
    result = asset_service.delete_final_asset(task_id=task_id, asset_id=asset_id, db=db, store=store)
...
def test_failed_deterministic_upload_is_queued_instead_of_deleted(tmp_path: Path) -> None:
    ...
    with (
        ...
        patch.object(asset_service, "queue_pending_storage_delete") as queue_delete,
...
def test_pending_delete_is_canceled_when_storage_key_is_referenced_again() -> None:
    ...
    row = AppSetting(
        key=asset_service.pending_storage_delete_key(storage_key),
    ...
    deleted = asset_service.retry_pending_storage_deletes(db, store)
