...
- **RAG 翻译 Agent**：字幕翻译前先扫描当前 block 和前文 summary，判断哪些术语真的需要查知识库或联网检索，减少把基础词、局部变量写入长期知识库。
- **PostgreSQL + pgvector 知识库**：术语、译法、来源、置信度和使用次数统一入库；支持向量检索、手动新增/删除条目、重建 embedding。
- **Wikipedia Tool + SearXNG Tool**：Agent 优先查固定 English Wikipedia API，必要时再调用自建 SearXNG，并可读取网页正文作为证据。
- **并发术语研究 Agent**：可在 `Settings -> Translate` 设置同时研究的术语数量；每个 Agent 都有独立工具调用 trace。
- **保守入库策略**：summarize -> verify -> write_rag；只有 verifier 判断来源支撑、上下文一致且置信度足够时才正式入库，否则标记为 pending、context_only 或 skipped。
- **独立 embedding 配置**：翻译 API key/base URL 与 embedding API key/base URL 分开；embedding 支持 OpenAI 兼容服务和本地模型。
...
- RAG Agent 支持固定 Wikipedia Tool、SearXNG 搜索、网页正文读取、独立 verifier、保守入库策略，并可在 Dashboard 查看每个 Agent 的工具调用 trace。
...
- 可在 `Settings -> Translate` 调整 Agent 并发数和超时时间。并发越高，术语补全越快，但外部 API、LLM 和搜索服务压力也越大。
