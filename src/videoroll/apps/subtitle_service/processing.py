...
def render_burn_in(
    ...
    *,
    ...
    if use_nvidia_gpu:
        ...
        if encoder is None:
            raise ValueError("NVIDIA GPU burn-in currently supports only h264/hevc/av1")
        if not _ffmpeg_supports_encoder(ffmpeg_path, encoder):
            raise RuntimeError(f"Current ffmpeg build does not support {encoder}")
        effective_cq = 24 if crf is None else max(0, min(51, int(crf)))
        preset_s = str(preset or "").strip().lower()
        nvenc_presets = {"p1", "p2", "p3", "p4", "p5", "p6", "p7"}
        if preset_s in nvenc_presets:
            effective_preset = preset_s
        else:
            # Keep historical software preset input wire-compatible while
            # translating it to NVENC's speed/quality scale.
            effective_preset = {
        ...
        if codec == "av1":
            video_args.extend(["-multipass", "qres"])
