...
首版目标是 Windows 10/11 x64 + NVIDIA GPU。客户端使用 nvidia-smi 枚举显卡，并通过随客户端分发的 FFmpeg 探测 h264_nvenc、hevc_nvenc、av1_nvenc。Coordinator 只把 RenderJob 分配到 Node；具体 GPU 仍由 Windows Node 自己选择。
...
6. 确认目标 NVIDIA GPU 和 NVENC 编码器出现后，点击“启动节点”。
